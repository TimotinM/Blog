...
namespace BlogCoreEngine.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
             ...
             var blogs = await context.Blogs.Include(x => x.Authors).Where(x => x.Authors.Select(x =>x.UserId).Contains(curUser.AuthorId)).ToListAsync(); 
             var posts = blogs.Select(x => x.Posts).SelectMany(x => x).ToList();

            SearchViewModel search = new SearchViewModel
            {
                Blogs = await this.blogRepository.GetAll(),
                Posts = (System.Collections.Generic.IReadOnlyList<PostDataModel>)posts
            };
